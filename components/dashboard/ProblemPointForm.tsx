...
import {
    ...
    gradeLabels,
    majorLabels,
} from "@/components/auth/createUser/formConstants";
...
export default function ProblemPointForm({ session }: { session: any }) {
    ...
    useEffect(() => {
        const fetchStudents = async () => {
            ...
            try {
                ...
                const res = await axios.get(
                    `/api/student/list-students-by-class?grade=${grade}&major=${major}&classNumber=${classNumber}`
                );
        ...
        fetchStudents();
    }, [selectedClass.grade, selectedClass.major, selectedClass.classNumber, isClassSelected]);
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-white p-6 rounded-xl border shadow-sm">
                <h2 className="text-xl font-semibold mb-4 text-gray-800">1. Select Class</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Select
                        value={selectedClass.grade}
                        onValueChange={(val) => setSelectedClass({ ...selectedClass, grade: val })}
                    >
                        <SelectTrigger>
                    ...
                    <Select
                        ...
                        onValueChange={(val) => setSelectedClass({ ...selectedClass, major: val })}
                    >
                        <SelectTrigger>
            ...
            <div className={`transition-opacity duration-300 ${!isClassSelected ? "opacity-50 pointer-events-none" : "opacity-100"}`}>
                <form onSubmit={handleSubmit} className="space-y-8">

                    {/* Student Selection */}
                    <div className="bg-white p-6 rounded-xl border shadow-sm">
                        <h2 className="text-xl font-semibold mb-4 text-gray-800">2. Select Students</h2>
                        {fetchingStudents ? (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-h-[300px] overflow-y-auto p-2 border rounded-md">
                                    {students.map((student) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <span className="font-medium truncate mr-2">{student.name}</span>
                                                {isSelected && <CheckCircle2 className="w-5 h-5 text-blue-600 flex-shrink-0" />}
                                            </div>
                                        );
                                    })}
                                </div>
                                <p className="text-sm text-gray-500 text-right">{selectedStudentIds.length} students selected</p>
                            </div>
                        ) : (
                            <div className="text-center p-8 text-gray-500">
                                {isClassSelected ? "No students found in this class." : "Select a class to see students."}
                            </div>
                        )}
                    </div>
                    ...
                    {/* Details Section */}
                    <div className="bg-white p-6 rounded-xl border shadow-sm">
                        <h2 className="text-xl font-semibold mb-4 text-gray-800">3. Problem Details</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium">Points to deducted</label>
                                <Input
                                    type="number"
                                    min={5}
                                    placeholder="Min. 5"
